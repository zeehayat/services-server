...
class VultrProvider(CloudProvider):
    ...
    def provision_server(self, request: ProvisionRequest) -> ProvisionResponse:
        ...
        return ProvisionResponse(
            ...
            ip_address="45.32.1.2",
            cost_per_month=cost,
            status="provisioned"
        )

    def get_cost(self, tier: InstanceTier, region: str) -> float:
        # Mock low cost for Vultr
        costs = {
            InstanceTier.PLATFORM_SMALL: 10.0,
            InstanceTier.PLATFORM_MEDIUM: 20.0,
            InstanceTier.PLATFORM_LARGE: 40.0,
        }
        return costs.get(tier, 0.0)

    def get_latency(self, region: str) -> float:
        # Mock latency for Vultr
        return 20.0
